...
static void test_embedded_array_start(void **_CBOR_UNUSED(state)) {
  assert_int_equal(1, cbor_encode_array_start(1, buffer, 512));
  assert_memory_equal(buffer, ((unsigned char[]){0x81}), 1);
}

static void test_array_start(void **_CBOR_UNUSED(state)) {
...
static void test_indef_array_start(void **_CBOR_UNUSED(state)) {
...
static void test_indef_array_encoding(void **_CBOR_UNUSED(state)) {
