...
static void test_embedded_bytestring_start(void **UNUSED(state)) {
  assert_int_equal(1, cbor_encode_bytestring_start(1, buffer, 512));
  assert_memory_equal(buffer, ((unsigned char[]){0x41}), 1);
}

static void test_bytestring_start(void **UNUSED(state)) {
...
static void test_indef_bytestring_start(void **UNUSED(state)) {
