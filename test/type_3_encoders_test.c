...
static void test_embedded_string_start(void **_CBOR_UNUSED(state)) {
  assert_int_equal(1, cbor_encode_string_start(1, buffer, 512));
  assert_memory_equal(buffer, ((unsigned char[]){0x61}), 1);
}

static void test_string_start(void **_CBOR_UNUSED(state)) {
...
static void test_indef_string_start(void **_CBOR_UNUSED(state)) {
