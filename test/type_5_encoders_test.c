...
static void test_embedded_map_start(void **_CBOR_UNUSED(state)) {
  assert_int_equal(1, cbor_encode_map_start(1, buffer, 512));
  assert_memory_equal(buffer, ((unsigned char[]){0xA1}), 1);
}

static void test_map_start(void **_CBOR_UNUSED(state)) {
...
static void test_indef_map_start(void **_CBOR_UNUSED(state)) {
