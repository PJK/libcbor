...
bool static cbor_claim_bytes(size_t required, size_t provided,
...
struct cbor_decoder_result cbor_stream_decode(
  ...
  if (!cbor_claim_bytes(1, source_size, &result)) {
    return result;
  }
  ...
  switch (*source) {
    ...
    case 0x19:
      ...
      {
        if (cbor_claim_bytes(2, source_size, &result)) {
    ...
    case 0x1A:
      ...
      {
        if (cbor_claim_bytes(4, source_size, &result)) {
    ...
    case 0x1B:
      ...
      {
        if (cbor_claim_bytes(8, source_size, &result)) {
    ...
    case 0x57:
      /* Embedded length byte string */
      {
        ...
        if (cbor_claim_bytes(length, source_size, &result)) {
